...
**结构层专项检测（对应朱雀检测 burstiness 指标）：**
- 扫描全文章段字数，若连续 5 段字数在 40-80 字区间，标记为”段落均匀——AI特征，需打破”。
- 统计单句段占比，若 < 20%，标记为”单句段不足——节奏单调，需增加短句爆发点”。
- 统计最长段与最短段字数比，若 < 5:1，标记为”段落缺乏疏密对比——需制造信息密度峰谷”。
- 扫描”不是……（而）是……”否定对比结构，全章 ≥ 4 次 → 标记为”否定对比泛滥——AI句式”。
- 扫描”不对。/不。/错了。”独立短句否定+纠正结构（否定词独立成句，下一句紧接纠正），全章 ≥ 1 次 → 标记为”独立短句否定——AI句式”，直接改写为正面陈述。
- 扫描”意味着什么——意味着”字面模式 → 命中即标记，改写为正面陈述。
- 扫描动作句+紧跟解释句（正则特征：展示走位/攻击后下一句以策略分析/含义解释开头）→ 找到 ≥ 3 对标记，删解释句或改为角色内心反应。
- 扫描抽象判断先行句（”这是……的表情””这代表着……””很显然……””毫无疑问……”）→ ≥ 3 次/章标记。
...
- 单句成段必须占比 ≥ 20%（冲突章节 ≥ 25%），不达标 = `anti_ai_force_check=fail`。
- 段落长度建议 20-100 字，避免连续大段说明。
- 场景切换优先用动作/声音/位置变化，不用”第一幕/第二幕”标签。
- 全章段落字数标准差应 ≥ 15，不达标需记录 deviation 并说明原因。
...
**结构层通过标准（新增）：**
- 单句成段占比 ≥ 20%，不达标 = 强制 fail。
- “不是 A 是 B”句内否定对比全章 = 0 次（出现即改写，无例外）。
- “不对。/不。/错了。”独立短句否定+纠正全章 = 0 次（出现即改写，无例外）。
- “意味着什么——意味着”字面模式 = 0 次（命中必须改写）。
- 展示+紧跟解释 ≤ 2 对（≥ 3 对必须改写至达标）。
- 连续段落均匀（5 段同在 40-80 字）必须打破至少 1 处。
- 冗余修饰堆叠：名词后连续 2+ 个修饰短语（”X极X，Y极Y”）→ 保留1个删其余。
- 切碎动作：同一角色的连续动作拆成3句以上短句 → 合成连贯句。
- 虚假伏笔：普通道具/动作用”没丢””那是唯一一把””他记住了”收尾 → 删掉尾句。
- 配角面板：路人/敌方展示完整HP/SP数值条 → 只保留等级和职业。
- AI面部微表情：”嘴角弧度””脚步间距””眸中闪过””瞳孔微缩” → 替换为具体行为。
