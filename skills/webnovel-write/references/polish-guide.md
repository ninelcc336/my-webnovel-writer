...
**必须先执行**：Read `${CLAUDE_PLUGIN_ROOT}/references/anti-ai-registry.md`，提取所有 `polish-guide` 视图。将每个 pattern 的 `rewrite` 规则和 `pass_criteria` 纳入 Anti-AI 终检。注册表模式是对以下7层基线规则的**增补**，不可替代基线。
...
- **注册表句式/结构层模式**：逐条应用注册表中 `polish-guide.layers` 包含 2 或 6 的所有 pattern 的 detection 规则。
...
- **注册表模式通过标准**：逐条应用注册表中每个 `polish-guide` 视图的 `pass_criteria`。核心红线：P001/P002/P003 = 0次，其余按各自阈值执行。
