...
产出可发布章节到 `正文/第{NNNN}章-{title}.md`。默认 2200-2500 字，用户/大纲另有要求时从之。
...
Agent(
  ...
  prompt: "chapter={chapter_num}; project_root=${PROJECT_ROOT}; scripts_dir=${SCRIPTS_DIR}; skill_root=${SKILL_ROOT}; storage_path=${PROJECT_ROOT}/.webnovel; state_file=${PROJECT_ROOT}/.webnovel/state.json（projection/read-model，仅兼容读取）。先 research，再按 本章硬性约束→CBN/CPNs/CEN→本章禁区→风格指引→dynamic_context补充参考 的顺序输出五段写作任务书。必须读取 style-target.md（路径 ${SKILL_ROOT}/references/style-target.md）并将其量化指标写入任务书第4段。"
)
```
...
**写前铁律卡**（起草前必须过一遍。以下为叙事姿态层约束——润色改不了的，写的时候就必须对）：

1. **否定前置**：直接写正面。不写"不是/不对/不像/没有X——Y"。
2. **不解释动作**：写完动作立即接后果或下一动作。不在动作后插入含义说明。
3. **不分析话语**：角色说完话或做完表情就完。不跟在后面翻译成文字含义。
4. **不测量**：不写"沉默了三秒""短促的笑""比昨天更轻"。不加"没到X的程度"等虚假程度限定。
5. **不替读者总结**：系统通知/对话已经展示过的信息，叙述不再复述一遍。不用单字句（"他让。""他忍。"）点破角色的决策。
6. **不用孤立名词过渡**：段落之间不单独放一个名词成段。过渡必须带角色的反应。
7. **环境描写有功能**：不写无目的的氛围填充（阳光从哪斜进来、灰尘怎么飘、盯着某物看了多久）。环境描写只写角色此刻注意到的，或影响角色行为的。

**每段写完后自查**：扫一眼刚写的段落——破折号后面是不是跟了句解释（"——这是他……""——很短""——不是被……"）？动作后面是不是塞了句含义翻译（"像在说……""这意味着……"）？句末有没有多余的确认词（"关不掉""没人回答"）？有就直接删。

> 具体句式检测（P001-P036完整规则）由 Step 3 reviewer + Step 4 polish 负责。起草时只需守住以上5条叙事姿态铁律。
