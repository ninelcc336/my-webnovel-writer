...
- `Read`：读取正文、设定集、记忆数据、**反AI注册表**
- `Grep`：在正文中搜索关键词
- `Bash`：调用记忆模块查询

**必须读取**：审查开始前先 Read `${CLAUDE_PLUGIN_ROOT}/references/anti-ai-registry.md` 获取所有 reviewer 视图的检查规则。
...
**必须先执行**：Read `${CLAUDE_PLUGIN_ROOT}/references/anti-ai-registry.md`（若路径不可达则尝试 `${SCRIPTS_DIR}/../references/anti-ai-registry.md`），提取所有 `reviewer` 视图的 pattern。对每个 pattern，按 `section` 分组，应用其 `detection`、`severity`、`threshold` 规则执行检查。

按 5 个子维度组织检查结果：

#### 6.1 词汇层
- 执行注册表中 section=6.1 的所有 pattern 检测
- 注册表未覆盖的通用检查：高频 AI 词汇是否密集

#### 6.2 句式层
- 执行注册表中 section=6.2 的所有 pattern 检测（含各子编号）
- 注册表未覆盖的通用检查：连续同构句、同义反复

#### 6.3 叙事层
- 执行注册表中 section=6.3 的所有 pattern 检测
- 注册表未覆盖的通用检查：节奏是否匀速（无快慢之分）

#### 6.4 情感层
- 执行注册表中 section=6.4 的所有 pattern 检测
- 注册表未覆盖的通用检查：所有角色是否用同一套反应模板

#### 6.5 对话层
- 执行注册表中 section=6.5 的所有 pattern 检测
- 注册表未覆盖的通用检查：是否全员书面语、无口语特征
