...
#### 6.2.6 「不是A是B」句内否定对比检查
- 扫描正文中「不是……（而）是……」「不是X——是Y」「不（只）是……是……」句内否定对比结构
- 全章 ≥ 1 次 → severity: `high`（绝对禁止，出现即报）
- evidence 必须列出所有出现位置的原文引用

#### 6.2.6.1 「独立短句否定+纠正」检查
- 扫描正文中「不对。」「不。」「错了。」作为独立短句、后接纠正句的跨句否定结构
- 特征：否定词独立成句（以句号/感叹号结尾），下一句紧接纠正内容
- 典型形式：「不对。不能叫X。」「不。这是Y。」「错了。应该是Z。」
- 全章 ≥ 1 次 → severity: `high`（绝对禁止，出现即报）
- evidence 必须列出所有出现位置的原文引用
- 与 6.2.6 的区别：6.2.6 是句内否定对比（"不是A，是B"），本节是跨句独立否定短句+纠正

#### 6.2.7 「展示+紧跟解释」模式检查
- 识别"动作描写句 → 紧跟策略意义/含义解释句"的对子
- 特征：解释句通常以"这意味着""这个站位""这招""这一下"开头
- 全章 ≥ 3 对 → severity: `high`
- 1-2 对 → severity: `medium`

#### 6.2.8 「意味着什么——意味着」自问自答检查
- 扫描"意味着什么——意味着"字面模式
- 命中即标记，severity: `medium`
- 同时扫描"X意味着什么——"省略变体

#### 6.2.9 段落均匀度检查（对应朱雀 burstiness 指标）
- 扫描全章连续段落字数，若 ≥ 5 段字数落在 40-80 字区间 → severity: `medium`
- 统计单句段占比，若 < 20% → severity: `high`
- 统计全章段落字数标准差（近似估计），若 < 10 → severity: `medium`

#### 6.2.10 新增反AI模式检查（来自人类编审反馈）
- 冗余修饰堆叠：「名词+修饰A+修饰B」结构（"字体极细，语气极平""蓝光安静地浮着"）→ 全章 ≥ 3 处 severity: `medium`
- 切碎动作：同一角色连续动作用句号拆成 ≥ 3 个独立短句（"捡起。换上。收进。"）→ severity: `medium`
- 虚假伏笔：普通道具/动作用"没丢""那是唯一一把""他记住了"收尾 → ≥ 2 处 severity: `medium`
- 配角面板数值化：路人/敌方展示完整 HP/SP 数值 → severity: `medium`
- AI面部微表情：「嘴角弧度」「脚步间距」「眸中闪过」「瞳孔微缩」→ ≥ 2 处 severity: `high`
